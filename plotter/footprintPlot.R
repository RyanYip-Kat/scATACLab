...
tryCatch({ p=PlotFootprint(object = seurat,
                       features=motif.names,
		       label.top=3,
		       assay=args$assay,
                       group.by=args$groupby)+patchwork::plot_layout(ncol = 1)
                   fileName=file.path(outDir,"motifs.pdf")
                   ggsave(fileName,plot=p,width=8,height=16)},
                   error=function(e){cat(sprintf("INFO : Invalid Motifs\n"))},
                   finnally={cat(sprintf("INFO : footprint plot [ %d ] sucessfuly!\n",length(motif.names)))}
        )
