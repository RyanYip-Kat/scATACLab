...
getDORCX=function(project,colCellType="Clusters",dorcCutoff=10){
	...
	genes=getFeatures(project,useMatrix="GeneIntegrationMatrix")
	geneSE=getMatrixFromProject(project,useMatrix="GeneIntegrationMatrix")
	rownames(geneSE)=genes
	...
	geneCounts=assay(geneSE[df$genes,])  # gene x  cell
	geneCounts=apply(geneCounts,2,MinMaxScale)
	...
	for(i in 1:nrow(df)){
		...
		idxG=dorcP2G$idxATAC[dorcP2G$idxRNA==G]
                peakG=assay(peakSE[idxG,])
                dorcG=colSums(peakG)/nFrags
                dorcGDF=data.frame("DORC"=dorcG)
                colnames(dorcGDF)=gene
                dorcDF_list[[i]]=dorcGDF
	}
	dorcDF=do.call(cbind,dorcDF_list)  # cell x gene
	#residualMat=as.matrix(geneCounts)-t(dorcDF)
	dorcDF=apply(dorcDF,1,MinMaxScale)  # gene x cell
	residualMat=as.matrix(geneCounts)[rownames(dorcDF),]-dorcDF
	...
	for(i in seq_along(uCellType)){
		...
		cellMatDF=data.frame("SCORE"=rowMeans(cellMat)) # nGene x 1
	...
	celltypeDorcDF=apply(celltypeDorcDF,2,MinMaxScale)
	return(list("DORCX"=dorcDF,"CellTypeDORCX"=celltypeDorcDF,"Residual"=residualMat))
}
	
projHeme=loadArchRProject("../ArchR_Result/CEpi_Mouse/Save-ProjHeme-P2G/")
dorcList=getDORCX(projHeme)
saveRDS(dorcList,"DORCXs.rds")
