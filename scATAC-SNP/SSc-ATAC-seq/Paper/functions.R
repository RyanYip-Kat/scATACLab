...
seuratSNN <- function(matSVD, dims.use = 1:50, print.output = TRUE,resolution=0.8){
  ...
  return(paste("Cluster",clust,sep=""))
}
...
seuratLSI <- function(mat, nComponents = 50, binarize = TRUE, nFeatures = NULL){
  ...
  mat <- mat[1:100,] + 1
  obj <- CreateSeuratObject(mat, project='scATAC', min.cells=0, min.features=0)
  colnames(matSVD)=paste("PCA",1:ncol(matSVD),sep ="_")
  obj[["pca"]]<-CreateDimReducObject(embeddings =matSVD,
                                  key = paste0("pca","_"),
...
addClusters <- function(obj, minGroupSize = 50, dims.use = seq_len(50), initialResolution = 0.8){
  ...
  obj <- FindNeighbors(obj,reduction="pca",dims =dims.use)
  obj <- FindClusters(object = obj,resolution = currentResolution,verbose = FALSE)
  minSize <- min(table(obj@meta.data[[paste0(Assay,"_snn_res.",currentResolution)]]))
  nClust <- length(unique(paste0(obj@meta.data[[paste0(Assay,"_snn_res.",currentResolution)]])))
  ...
  while(minSize <= minGroupSize){
    obj@meta.data <- obj@meta.data[,-which(colnames(obj@meta.data)==paste0(Assay,"_snn_res.",currentResolution))]
