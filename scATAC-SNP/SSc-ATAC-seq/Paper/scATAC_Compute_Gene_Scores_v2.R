...
gtfFile <- "/home/ye/Data/10X/VDJ/ref/refdata-cellranger-GRCh38-3.0.0/genes/genes.gtf"
...
saveRDS(ciceroObj,file.path(outDir, "cicero-aggregated-accessibility-cds.rds"))

#Keep Cell Mappings!
cellMapKNN <- ciceroOut[[2]]
saveRDS(cellMapKNN, file.path(outDir,"cicero-KNN-Groupings-cds.rds"))
...
saveRDS(connections,file.path(outDir,"connections.rds"))
#Annotate CDS
message("Annotating Cell Data Set...")

gene_anno <- rtracklayer::readGFF(gtfFile)
...
obj <- annotate_cds_by_site(obj, gene_annotation_sub)
#### Generate gene activity scores ####
# generate unnormalized gene activity matrix
message("Build gene activity matrix...")
unnorm_ga <- build_gene_activity_matrix(obj,connections,coaccess_cutoff = corCutOff)
...
num_genes <- pData(obj)$num_genes_expressed
names(num_genes) <- row.names(pData(obj))
print(head(num_genes))
...
ciceroGA <- normalize_gene_activities(unnorm_ga, num_genes)
print(head(rownames(ciceroGA)))
saveRDS(ciceroGA,file.path(outDir,"ciceroGA.rds"))


message("Make TSS Window for genes...")
...
seCicero <- SummarizedExperiment(
    ...
    colData = mdata[colnames(ciceroGA),]
)
...
saveRDS(seCicero, file.path(outDir,"Cicero-Gene-Activity.rds"))
saveRDS(seCiceroLog, file.path(outDir,"Cicero-Log2-Gene-Activity.rds"))
