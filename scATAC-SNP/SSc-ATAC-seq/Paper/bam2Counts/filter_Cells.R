...
parser$add_argument("--fragment",
                    type="character",
                    default=NULL,
                    help="path of fragment file")


parser$add_argument("--name",
		    type="character",
		    default="PBMC",
		    help="prefix name for output object")
...
filterFrags <- 1000
filterTSS <- 8
file_fragments <- args$fragment
out_fragments <- file.path(outDir,paste0(name,"-fragments.gr.rds"))
...
#pdf(file.path(outDir,paste0(name,"_Filter-Cells.pdf")))
...
#write.table(tssSingles,file.path(outDir, paste0(name,"_Filter-Cells.txt")))
...
saveRDS(fragments, file.path(outDir,out_fragments))
message("INFO : Done!")
