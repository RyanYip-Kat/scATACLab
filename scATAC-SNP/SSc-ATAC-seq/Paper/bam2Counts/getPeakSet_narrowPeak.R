...
countInsertions <- function(query, fragments,name, by = "score"){
  ...
  colnames(sparseM)=paste(name,1:ncol(sparseM),sep="-")
...
df <- data.frame(
  samples = basename(gsub("\\_summits.bed","",list.files(inPath, pattern = "\\_summits.bed", full.names = FALSE,recursive=TRUE))),
  groups = dirname(gsub("\\_summits.bed","",list.files(inPath, pattern = "\\_summits.bed", full.names = FALSE,recursive=TRUE))),
  summits = list.files(inPath, pattern = "\\_summits.bed",recursive=TRUE, full.names = TRUE)
  )
...
message("INFO : Make Counts ...")
narrowFiles=list.files(inPath,pattern="\\_peaks.narrowPeak",recursive=TRUE, full.names = TRUE)
Names = basename(gsub("\\_summits.bed","",list.files(inPath, pattern = "\\_summits.bed", full.names = FALSE,recursive=TRUE)))
message("INFO : Create Counts list")
countsPeaksList <- lapply(seq_along(narrowFiles), function(i){
  message(sprintf("%s of %s", i, length(narrowFiles)))
  gc()
  countInsertions(unionPeaks, import(narrowFiles[i]),name=Names[i],by = "name")
 })

message("INFO : CountsMatrix")
...
message("INFO : Create SummarizedExperiment Object")
se <- SummarizedExperiment(
  assays = SimpleList(counts = mat),
  rowRanges = unionPeaks
  )

Sample=unlist(lapply(colnames(se),function(name)return(stringr::str_split(name,"-")[[1]][1])))
rownames(se) <- paste(seqnames(se),start(se),end(se),sep="_")
colData(se)$FRIP <- frip
colData(se)$uniqueFrags <- total / 2
colData(se)$Sample=Sample

message("INFO : Save ATAC-Summarized-Experiment")
