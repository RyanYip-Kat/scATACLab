...
for(i in seq_along(arrowFiles)){
	...
	if(!file.exists(zipFrag)){
		...
		cat(sprintf("INFO : Write fragment into : [ %s ] \n",outFrag)) 
	        write.table(fragment[,c("seqnames","start","end","cells")],outFrag,sep="\t",row.names=F,quote=F,col.names=F)
...
message("INFO : Make Combined Peaks")
...
#reduceOwn=function(x){reduce(x,drop.empty.ranges=TRUE)}
#combined.peaks <- do.call(reduceOwn,GR_list)
#combined.peaks <- GR_list[[1]]
#for(i in 2:length(GR_list)){
#	cat(sprintf("INFO : [ %d of %d ]\n",i,length(GR_list))) 
#	gr=GR_list[[i#]]
#	combined.peaks=GenomicRanges::reduce(x=c(combined.peaks,gr))
#}
...
seurat=merge(x=combined_list[[1]],
	     y=combined_list[-1],
	     add.cell.ids=names(combined_list))
...
ReducedDimDir=file.path(outDir,"ReducedDimNames")
