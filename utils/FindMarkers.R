...
markerList <- getMarkers(MF, cutOff = "Pval <= 0.05 & Log2FC >= 1.0")
m=as.data.frame(markerList)
write.table(m,file.path(args$outdir,"MarkerList.csv"),sep=",",quote=F,row.names=F)

heatmapMF <- plotMarkerHeatmap(
  seMarker = MF,
  cutOff = "Pval <= 0.05 & Log2FC >= 1.0",
  labelMarkers = NULL,
  labelRows=FALSE,
  transpose = TRUE
)
MyplotPDF(heatmapMF, name = paste0(args$useMatrix,"-Marker-Heatmap2"), width = 16, height =12, outpath =args$outdir, addDOC = FALSE)
